 ...
 * Uses a native `title` (not MUI Tooltip) to stay hydration-safe in lists.
 */
export default function NoteDot({ summary }: { summary: NoteSummary | undefined }) {
  if (!summary) return null;
  const color = CATEGORY[summary.category] ?? '#888';
  const title = `${summary.count} linked note${summary.count > 1 ? 's' : ''}${
    summary.highPriority ? ' · high priority' : ''
  }`;
  return (
    <Box component="span" title={title} sx={{ display: 'inline-flex', alignItems: 'center' }}>
